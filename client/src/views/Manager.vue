            ...
            <div v-if="this.session().loggedin" class="content">
                <ul class="nav nav-pills" id="pills-tab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="pills-templates-alerts" data-bs-toggle="pill"
                            data-bs-target="#pills-alerts" type="button" role="tab"
            ...
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-alerts" role="tabpanel"
                    ...
                    <div class="p-0 m-1 border-0">
                        <div class="content">
                            ...
                            <ul v-if="this.notes.length > 0" class="list-group">
                                <div v-for="(ad, index) in this.notes">
                                    <li v-if="(this.session().loggedin && (ad.addressedTo === null || ad.addressedTo === this.session().name)) || ad.onBroadcast === true"
                                        :class="getClass(ad)">
                                        <div class="ms-1 me-1 row">
                                            <div
                                                class="fw col-12 col-md-10 col-xl-11 align-items-center text-align-center mb-2">
                                                <div class="row">
                                                    <div v-if="ad.onBroadcast" class="col-auto"> ★ </div>
                                                    <strong class="col-auto"> {{ ad.name }} </strong>
                                                    <small class="col-auto">
                                                        <div class="row justify-content-start">
                                                            <div v-if="ad.expires === '9999-01-01'" class="col-auto">
                                                                Актуально: бессрочно;
                                                            </div>
                                                            <div v-if="ad.expires !== '9999-01-01'" class="col-auto">
                                                                Актуально до: {{ ad.expires }};
                                                            </div>
                                                            <i class="col-auto"> {{ ad.author.name }} </i>
                                                        </div>
                                                    </small>
                                                </div>
                                                {{ ad.comment }}
                                            ...
                                            <div v-if="['admin', 'moderator', 'manager'].includes(this.session().role) || ad.addressedTo === this.session().name"
                                                class="col-12 col-md-2 col-xl-1">
                                                <span class="badge w-100 p-0"
                                                    style="position: relative; left: 50%; top: 50%; transform: translate(-50%, -50%);">
                                                    <button name="deleteNote" type="submit" @click="delNote(ad.id, index)"
                        ...
                        <div v-if="['admin', 'moderator', 'manager'].includes(this.session().role)">
                            <div class="content">
                                <div class="mb-3">
                                    <label for="nameIn" class="form-label">Уведомление</label>
                                    <input v-model="this.ad_name" type="text" class="form-control" id="nameIn" name="name"
                                        placeholder="Заголовок уведомления" />
                                </div>
                                <div class="mb-3">
                                    <label for="commentIn" class="form-label">Сообщение</label>
                                    <textarea v-model="this.ad_comment" class="form-control" id="commentIn" name="comment"
                                        rows="3" placeholder="Текст уведомления"></textarea>
                                </div>
                                <div class="row m-0 g-2 align-items-center">
                                    <div class="col-12 col-md-4 form-check form-switch">
                                        <input v-model="this.ad_translate" name="translate" class="form-check-input"
                                    ...
                                    <div class="col-12 col-md-4 form-check form-switch">
                                        <input v-model="this.ad_endless" name="unlimited" class="form-check-input"
                                            type="checkbox" role="switch" id="flexSwitchCheckChecked" />
                                        <label class="form-check-label" for="flexSwitchCheckChecked">Бессрочное
                                            объявление</label>
                                    </div>
                                    <div v-if="!ad_endless" class="col-12 col-md-4 p-0">
                                        <div class="col-12"> Актуально до: </div>
                                        <div class="col-12">
                                            <input v-model="this.ad_actual" name="time" value="" id="addDate"
                                                class="col form-control m-0" type="date"
                                                style="margin-left: 12px; margin-right: 12px" min="2023-01-26"
                                                max="2024-01-30" />
                                        </div>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        ...
        {
            return {
                'list-group-item list-group-item-info justify-content-between align-items-center': ad.addressedTo === this.session().name && ad.name === 'Запрос отправлен',
                'list-group-item list-group-item-danger justify-content-between align-items-center': ad.addressedTo === this.session().name && ad.name === 'Запрос отклонён',
