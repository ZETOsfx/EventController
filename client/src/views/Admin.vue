            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button name="id" form="del<%=i%>" type="submit" @click="deleteUser(this.forModal)"
        ...
        <div class="modal fade" id="ModalSaveProfile" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"> Отмена </button>
                        <button @click="saveChanges(this.forModal)" type="button" class="btn btn-success"
        ...
        <div class="container">
            ...
            <div class="content">
                ...
                <ul class="nav nav-pills" id="pills-tab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="pills-profiles-tab" data-bs-toggle="pill"
                            data-bs-target="#pills-profiles" type="button" role="tab" aria-selected="true">Пользватели в
                            системе</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-screens-tab" data-bs-toggle="pill"
                            data-bs-target="#pills-screens" type="button" role="tab" aria-selected="false">Экраны в
                            системе</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-scusers-tab" data-bs-toggle="pill"
                            data-bs-target="#pills-scusers" type="button" role="tab" aria-selected="false">Доступ к
                            экранам</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-subs-tab" data-bs-toggle="pill" data-bs-target="#pills-subs"
                            type="button" role="tab" aria-selected="false">Оповещения</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-logs-tab" data-bs-toggle="pill" data-bs-target="#pills-logs"
                            type="button" role="tab" aria-selected="false">Последние изменения</button>
                    </li>
                    <!-- КОНЕЦ: Вкладки -->
                </ul>
            </div>
            ...
            <div class="content">
                ...
                <div class="tab-content" id="pills-tabContent">
                        ...
                        <div class="introduce">
                            <div v-for="(user, index) in this.users">
                                <div class="row gy-2 gx-2 align-items-center" style="margin-top: 3px;">
                                    <!-- ОТОБРАЖАЕМОЕ ИМЯ -->
                                    <div @click="this.clickBlock(this.users[index].lock)"
                                        class="col-12 col-md-6 col-lg-4 col-xl-2">
                                        <input type="text" class="form-control" name="name" @change="this.isChanged = true"
                                            v-model="this.users[index].name" :disabled="this.users[index].lock" />
                                    </div>
                                    <!-- РЕДАКТИРОВАТЬ -->
                                    ...
                                    <div v-if="!user.lock" class="col-12 col-md-6 col-lg-4 col-xl">
                                        <input type="text" class="form-control" name="name" @change="this.isChanged = true"
                                            v-model="this.users[index].login" />
                                    </div>
                                    <!-- РОЛЬ -->
                                    <div v-if="!user.lock && user.name !== this.session().name"
                                        class="col-12 col-lg-4 col-xl">
                                        <select v-model="this.users[index].role" @change="this.isChanged = true"
                                            class="form-select" name="role" id="role">
                                            <option v-for="(vol, index) in this.volumes" :value="vol"
                                                :selected="user.role === vol"> {{ this.roles[index] }} </option>
                                        </select>
                                    </div>
                                    <!-- НОВЫЙ ПАРОЛЬ -->
                                    <div v-if="!user.lock" class="col-12 col-md-6 col-lg-4 col-xl">
                                        <div class="input-group">
                                            <input type="password" class="form-control" id="password>" name="password"
                                                placeholder="Новый пароль" readonly v-model="this.users[index].password"
                                                @change="this.isChanged = true"
                                    ...
                                    <div v-if="!user.lock" class="col-12 col-md-6 col-lg-4 col-xl">
                                        <div class="input-group">
                                            <input type="password" class="form-control" id="password" name="password"
                                                placeholder="Подтвердите пароль" readonly
                                                v-model="this.users[index].checkpass"
                                                onfocus="this.removeAttribute('readonly')" />
                                        </div>
                                    </div>
                                    <!-- СОХРАНИТЬ -->
                                    <div v-if="!user.lock" class="col-6 col-lg-2 col-xl">
                                        <button @click="triggerModal(index, 'save')" type="button"
                                            class="btn btn-outline-success w-100">Сохранить</button>
                                    </div>
                                    <!-- УДАЛИТЬ -->
                                    <div v-if="!user.lock" class="col-6 col-lg-2 col-xl">
                                        <button v-if="!(this.users[index].name === this.session().name)"
                                            @click="this.triggerModal(index, 'del')" type="button"
                                            class="btn btn-outline-danger w-100"> Удалить </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row gy-2 gx-2 align-items-center" style="margin-top: 3px;">
                                <div class="col-12 col-md-6 col-lg-4 col-xl-2">
                                    <input type="text" class="form-control" id="name" name="name"
                                        placeholder="Отображаемое имя" readonly v-model="this.addForm.name"
                                        onfocus="this.removeAttribute('readonly')" />
                                </div>
                                <div class="col-12 col-md-6 col-lg-4 col-xl">
                                    <input type="text" class="form-control" id="login" name="name" placeholder="Логин"
                                        readonly v-model="this.addForm.login" onfocus="this.removeAttribute('readonly')" />
                                </div>
                                ...
                                <div class="col-12 col-md-6 col-lg-4 col-xl">
                                    <div class="input-group">
                                        <input type="password" class="form-control" id="password" name="password"
                                            placeholder="Пароль" readonly v-model="this.addForm.password"
                                            onfocus="this.removeAttribute('readonly')" />
                                    </div>
                                </div>
                                <div class="col-12 col-md-6 col-lg-4 col-xl">
                                    <div class="input-group">
                                        <input type="password" class="form-control" id="password" name="password"
                                            placeholder="Подтвердите пароль" readonly v-model="this.addForm.checkpass"
                                            onfocus="this.removeAttribute('readonly')" />
                                    </div>
                                </div>
                    ...
                    <!-- Управление экранами -->
                    <div class="tab-pane fade" id="pills-screens" role="tabpanel" aria-labelledby="pills-screens-tab"
                        tabindex="0">
                    ...
                    <!-- НАЗНАЧЕНИЕ ЭКРАНОВ -->
                    <div class="tab-pane fade" id="pills-scusers" role="tabpanel" aria-labelledby="pills-scusers-tab"
                        tabindex="0">
                        <div class="d-flex align-items-start">
                            <div class="row ms-2">
                                <div class="nav flex-column nav-pills col-12 col-md-auto me-3 mt-2" id="v-pills-tab"
                                    role="tablist" aria-orientation="vertical">
                                    <!-- АДМИНИСТРАТОРЫ -->
                                    <button class="nav-link active" id="v-pills-admins-tab" data-bs-toggle="pill"
                                        data-bs-target="#v-pills-admins" type="button" role="tab"
                                        aria-controls="v-pills-admins" aria-selected="true">Администраторы</button>
                                    <!-- МОДЕРАТОРЫ -->
                                    <button class="nav-link" id="v-pills-moders-tab" data-bs-toggle="pill"
                                        data-bs-target="#v-pills-moders" type="button" role="tab"
                                        aria-controls="v-pills-moders" aria-selected="false">Модераторы</button>
                                    <!-- РЕДАКТОРЫ -->
                                    <button class="nav-link" id="v-pills-editors-tab" data-bs-toggle="pill"
                                        data-bs-target="#v-pills-editors" type="button" role="tab"
                                        aria-controls="v-pills-editors" aria-selected="false">Редакторы</button>
                                    <!-- МЕНЕДЖЕРЫ -->
                                    <button class="nav-link" id="v-pills-managers-tab" data-bs-toggle="pill"
                                        data-bs-target="#v-pills-managers" type="button" role="tab"
                                        aria-controls="v-pills-managers" aria-selected="false">Менеджеры</button>
                                </div>
                                <!-- СОДЕРЖИМОЕ ВКЛАДОК -->
                                <div class="tab-content col-12 col-md ps-0  mt-2" id="v-pills-tabContent">
                                    <!-- СОДЕРЖИМОЕ АДМИНИСТРАТОРЫ -->
                                    <div class="tab-pane fade show active" id="v-pills-admins" role="tabpanel"
                                        aria-labelledby="v-pills-admins-tab" tabindex="0">
                                        <ul class="list-group">
                                            <li class="list-group-item">
                                                <div class="row g-2">
                                                    <div class="col-12 col-md-7 col-lg-6 col-xl-5">
                                                        <input type="text" class="form-control" id="name" name="name"
                                                            value="Пользователь 1" disabled readonly>
                                                    </div>
                                                    ...
                                                    <div class="row row-cols-auto">
                                                        <div class="m-0 ms-2 p-0 b-0 mt-1">
                                                            <input type="checkbox" class="btn-check col-sm" id="btn-check-1"
                                                                checked>
                                                            <label class="btn btn-outline-success" for="btn-check-1">Кафедра
                                                                К3</label>
                                                        </div>
                                                        <div class="m-0 ms-2 p-0 b-0 mt-1">
                                                            <input type="checkbox" class="btn-check col-sm"
                                                                id="btn-check-2">
                                                            <label class="btn btn-outline-success" for="btn-check-2">МФ TV
                                                                HALL</label>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <li class="list-group-item">
                                                <div class="row g-2">
                                                    <div class="col-12 col-md-7 col-lg-6 col-xl-5">
                                                        <input type="text" class="form-control" id="name" name="name"
                                                            value="Пользователь 2" disabled readonly>
                                                    </div>
                                                    <div class="col-12 m-1">
                                                        Доступные экраны для работы в системе:
                                                    </div>
                                                    <div class="row row-cols-auto">
                                                        <div class="m-0 ms-2 p-0 b-0 mt-1">
                                                            <input type="checkbox" class="btn-check col-sm" id="btn-check-3"
                                                                checked>
                                                            <label class="btn btn-outline-success" for="btn-check-3">Кафедра
                                                                К3</label>
                                                        </div>
                                                        <div class="m-0 ms-2 p-0 b-0 mt-1">
                                                            <input type="checkbox" class="btn-check col-sm"
                                                                id="btn-check-4">
                                                            <label class="btn btn-outline-success" for="btn-check-4">МФ TV
                                                                HALL</label>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="tab-pane fade" id="v-pills-moders" role="tabpanel"
                                        aria-labelledby="v-pills-moders-tab" tabindex="0">
                                        <ul class="list-group">
                                            <li class="list-group-item">
                                                <div class="row g-2">
                                                    <div class="col-12">
                                                        <input type="text" class="form-control" id="name" name="name"
                                                            value="Нет доступных пользователей" disabled readonly>
                                                    </div>
                                                    <div class="col-12 m-1">
                                    ...
                                    <div class="tab-pane fade" id="v-pills-editors" role="tabpanel"
                                        aria-labelledby="v-pills-editors-tab" tabindex="0">
                                        <ul class="list-group">
                                            <li class="list-group-item">
                                                <div class="row g-2">
                                                    <div class="col-12">
                                    ...
                                    <div class="tab-pane fade" id="v-pills-managers" role="tabpanel"
                                        aria-labelledby="v-pills-managers-tab" tabindex="0">
                                        <ul class="list-group">
                                            <li class="list-group-item">
                                                <div class="row g-2">
                                                    <div class="col-12">
                    ...
                    <div class="tab-pane fade" id="pills-subs" role="tabpanel" aria-labelledby="pills-subs-tab"
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-6 col-lg-auto mt-2">
                                <button type="button" class="btn btn-outline-danger mb-0 w-100">Удалить почтовый
                                    адрес</button>
                            </div>
                        </div>
                    </div>
    ...
    methods: {
        ...
        async saveChanges(modalData)
        {
            ...
            if (this.users[index].name.length > 16) {
                ...
                if (this.isChanged) {
                    if (this.users[index].checkpass === this.users[index].password) {
