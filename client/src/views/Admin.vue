            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button name="id" form="del<%=i%>" type="submit" @click="delProfile(this.forModal)" class="btn btn-danger">Удалить</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="ModalSaveProfile" tabindex="-1" >
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"> Отмена </button>
                        <button @click="saveEditProfile(this.forModal)" type="button" class="btn btn-success" data-bs-dismiss="modal">Сохранить</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="introduce">
                            <div v-for="(user, index) in this.users">
                                <div class="row gy-2 gx-2 align-items-center" style="margin-top: 3px;">
                                    <!-- ОТОБРАЖАЕМОЕ ИМЯ -->
                                    <div @click="this.clickDebian(this.users[index].lock)" class="col-12 col-md-6 col-lg-4 col-xl-2">
                                        <input type="text"
                                            class="form-control"
                                            name="name" @change="this.isChanged = true"
                                            v-model="this.users[index].name" :disabled="this.users[index].lock"/>
                                    </div>
                                    <!-- РЕДАКТИРОВАТЬ -->
                                    <div v-if="user.lock" class="col-12 col-md-6 col-lg-4 col-xl"> 
                                        <button @click="startEditProfile(index)" class="btn btn-outline-secondary">
                                            Редактировать
                                        </button>
                                    </div>
                                    <!-- ЛОГИН -->
                                    ...
                                    <div v-if="!user.lock" class="col-12 col-md-6 col-lg-4 col-xl">
                                        <div class="input-group">
                                            <input type="password"
                                                ...
                                                v-model="this.users[index].password"
                            ...
                            <div class="row gy-2 gx-2 align-items-center" style="margin-top: 3px;">
                                <div class="col-12 col-md-6 col-lg-4 col-xl-2">
                                    <input type="text"
                                        class="form-control"
                                        id="name" name="name"
                                        placeholder="Отображаемое имя" readonly
                                        v-model="this.addForm.name"
                                        onfocus="this.removeAttribute('readonly')"/>
                                </div>
								...
								<div class="col-12 col-md-6 col-lg-4 col-xl">
								  <div class="input-group">
									<input type="password"
										   ...
										   id="password" name="password"
										   placeholder="Подтвердите пароль" readonly
										   v-model="this.addForm.checkpass"
										   onfocus="this.removeAttribute('readonly')"/>
								  </div>
								</div>
                                <div class="col-6 col-lg-2 col-xl">
                                    <button type="submit" @click="addProfile" class="btn btn-success">Добавить</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tab-pane fade" id="pills-scusers" role="tabpanel" aria-labelledby="pills-scusers-tab" tabindex="0"> 
                        <div class="d-flex align-items-start">
                            <div class="row ms-2">
                                ...
                                <!-- МЕНЕДЖЕРЫ -->
                                <button class="nav-link" id="v-pills-adsenders-tab" data-bs-toggle="pill" data-bs-target="#v-pills-adsenders" type="button" role="tab" aria-controls="v-pills-adsenders" aria-selected="false">Менеджеры</button>
                                </div>
                                <!-- СОДЕРЖИМОЕ ВКЛАДОК -->
                                ...
                                <div class="tab-pane fade" id="v-pills-moders" role="tabpanel" aria-labelledby="v-pills-moders-tab" tabindex="0"> 
                                    ...
                                    <li class="list-group-item">
                                        ...
                                        <div class="col-12 m-1">
                                        Добавьте хотя бы одного пользователя в систему с данной ролью для настройки прав доступа к экранам.
                                        </div>
                                        </div>
                                    </li>
                                    </ul>
                                ...
                                <div class="tab-pane fade" id="v-pills-adsenders" role="tabpanel" aria-labelledby="v-pills-adsenders-tab" tabindex="0">
                                    <ul class="list-group">
                                    <li class="list-group-item">
                                        <div class="row g-2">
                                        <div class="col-12">
...
export default {
	...
	data() {
		return {
			...
			users: [],
			volumes: [ "editor", "moder", "adsender", "admin"],
	...
	methods: {
		...
		async getProfiles() {
			...
			this.delProfileModal = new bootstrap.Modal(document.getElementById('ModalDeleteProfile'));
            ...
            // Ты взбесил модалку
		async triggerModal(index, delorsave) {
			...
			if (delorsave === 'save') {
				if (this.isChanged)
					...
					await this.saveEditProfile(this.forModal);
			} else if (delorsave === 'del') {
				this.delProfileModal.show();
			} else {
				console.log('error');
			}
		},
		...
		async addProfile() {
			...
			if (this.addForm.name.length > 16) {
				this.errorMessage = 'Длина отображаемого имени превышает ограничение: 16 символов.';
				this.errCallback.show();
			} else if (this.addForm.login.length > 20) {
				this.errorMessage = 'Длина логина превышает ограничение: 20 символов.';
				this.errCallback.show();
			} else {
					// OK
                ...
                let response = await fetch(`/account/register`, this.options('PATCH', {
		...
		async startEditProfile(index) {
            await this.getTime();   
            if (!this.process_started) {
                this.users[index].lock = false;
                this.process_started = true;
            } else {
                this.errorMessage = 'Во имя избежания ошибок сохранения одновременно можно редактировать не более одного профиля.';
				this.errCallback.show();
            }
		},
			// Сохранить изменения в профиле
		async saveEditProfile(obj) {
			await this.getTime();
			if (this.users[obj.index].name.length > 16) {
				this.errorMessage = 'Длина отображаемого имени превышает ограничение: 16 символов.';
				this.errCallback.show();
			} else if (this.users[obj.index].login.length > 20) {
				this.errorMessage = 'Длина логина превышает ограничение: 20 символов.';
				this.errCallback.show();
			} else {
				if (this.isChanged) {
                    if (this.users[obj.index].checkpass === this.users[obj.index].password) {
                        console.log(this.users[obj.index]);
                        let response = await fetch(`/account/upd`, this.options('PATCH', {
                            id: this.users[obj.index].id,
                            name: this.users[obj.index].name,
                            login: this.users[obj.index].login,
                            role: this.users[obj.index].role,
                            password: this.users[obj.index].password
                        }));
                        response = await response.json();

                        if (response.result === 'success') {
                            // toast success: result.message
                            this.users[obj.index].password = '';
                        ...
                        this.successMessage = 'Изменения внесены успешно.';
                        this.succCallback.show();
                    } else {
                        this.errorMessage = 'Введенные пароли не совпадают.';
				        this.errCallback.show();
				...
				this.users[obj.index].lock = true;
			}
		},
			// Стереть профиль
		async delProfile(obj) {
            await this.getTime();
			await fetch(`/account/rm`, this.options('POST', { id: this.users[obj.index].id }));
			this.users.splice(obj.index, 1);
			this.delProfileModal.hide();
            this.successMessage = 'Профиль был успешно удален.';
            this.succCallback.show();
            this.process_started = false;
		},
            // Опции
		options(method, body) {
            ...
            // Марсель кликает на блокд ссылку
		clickDebian(isDebian) {
		  	this.getTime();
			if (isDebian) {
				this.errorMessage = 'Начните редактирование формы для изменения данного параметра.';
				this.errCallback.show();
			}
		},
	},
