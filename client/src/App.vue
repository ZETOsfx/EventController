...
export default {
    ...
    data() {
        return {
            socket: io('ws://' + window.API_URL.substring(7)),
    ...
    methods: {
         ...
         */
        connect() {
            this.socket = io('ws://' + window.API_URL.substring(7));
        ...
        browserDetect() {
            ...
            if ((this.browserName === 'Firefox' && Number(this.browserVersion.split('.')[0]) < 84) || (this.browserName === 'Chrome' && Number(this.browserVersion.split('.')[0]) < 80) || (this.browserName === 'Microsoft Edge' && Number(this.browserVersion.split('.')[0]) < 88) || (this.browserName === 'Opera' && Number(this.browserVersion.split('.')[0]) < 66) || (this.browserName === 'Yandex Browser' && Number(this.browserVersion.split('.')[0]) < 20) || (this.browserName === 'Safari' && Number(this.browserVersion.split('.')[0]) < 14)) {
        ...
        <nav class="sidebar close" v-if="this.$route.name !== 'Broadcast' && this.$route.name !== 'Adscast'">
            <header>
                <div class="image-text">
                    ...
                    <div class="text logo-text">
                        ...
                        <span v-if="this.session.loggedin && this.session.name" class="profession ms-2" style="max-width: 185px">
                            {{ this.getRole() }}
                        </span>
            ...
            <div class="menua-bar">
                <div class="menua">
                    ...
                    <li class="nav-linka">
                        <router-link @click="closeTab('note')" class="position-relative" to="/note">
                            ...
                            <span v-if="this.session.unread > 0" class="position-absolute top-40 start-30 translate-middle badge rounded-pill bg-danger" style="margin-left: 40px">
                                {{ this.session.unread }}
                            </span>
                ...
                <div class="bottom-content">
                    ...
                    <li class="">
                        <router-link @click="onLogout()" v-if="this.session.loggedin && this.session.name" class="position-relative" to="/login">
                            <i class="bx bx-log-out icon"></i>
                            <span class="text nav-text">Выйти</span>
                        </router-link>
                        <router-link @click="closeTab()" v-if="!this.session.loggedin || !this.session.name" class="position-relative" to="/login">
